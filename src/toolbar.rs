...
use druid::{widget::{Flex, Button, ControllerHost, Padding, Click, DisabledIf, TextBox, 
    Container, Label, Slider, KnobStyle, Axis, LensWrap, SizedBox, RawLabel, LineBreaking, Scroll}, 
    FileSpec, FileDialogOptions, WidgetExt, WindowDesc, Lens, Screen, KeyOrValue, Color, text::{RichTextBuilder, Attribute, RichText}, FontFamily, TextAlignment};
use image::GenericImageView;

// IMPORTAZIONI INTERNE
use crate::{ebook::{BookState, ViewState, book_state_derived_lenses::fontSize, reactToBarSearchRefresh}, 
    constants::{GO_PREV, GO_NEXT, JUMP_BY_BUTTON, EPUB_LOADING, PURPLE, ROUNDED_VALUE, BLACK, EMPTY_STRING, IMAGE_LOADING, BIG_SPACER, TINY_SPACER, MAX_ZOOM, MIN_ZOOM}, 
    search::{looksForMatchesInPage, reverseOCR, looksForMatchesInChapter}, event::ZoomEvent};
...
fn textSearchButton () -> DisabledIf<BookState, Padding<BookState, ControllerHost<Button<BookState>, Click<BookState>>>>{
    ...
    Button::new("🔍")
        .on_click(|_, bookState: &mut BookState, _| {
            if bookState.getCurrentView() == ViewState::ReadMode {
                looksForMatchesInPage(bookState, &bookState.getBarText());
            }if bookState.getCurrentView() == ViewState::EditMode {
    
                looksForMatchesInChapter(bookState, &bookState.getBarText());
            }
        })
        .padding(2.0)
...
#[allow(non_snake_case)]
fn refreshSearchButton () -> DisabledIf<BookState, Padding<BookState, ControllerHost<Button<BookState>, Click<BookState>>>>{

    Button::new("🔄")
        .on_click(|_, bookState, _| {
            reactToBarSearchRefresh(bookState);
        })
        .padding(2.0)
        .disabled_if(|bookState: & BookState, _| 
            bookState.getCurrentView() == ViewState::Idle
...
fn ocrReverseButton () -> DisabledIf<BookState, Padding<BookState, ControllerHost<Button<BookState>, Click<BookState>>>> {
    ...
    Button::new("🖼️")
        .on_click(move |ctx, bookState : & mut BookState, _env| {
            ...
            let rect= Screen::get_display_rect();
            let center = druid::Point::new((rect.x1 as u32 - imgWidth) as f64 /2 as f64, 0 as f64);
            let mut msg = String::from("\nExtimeted paper page: ");
            msg.push_str(bookState.getDigitalPageEvaluetion().to_string().as_str());
            
            let mut builder = RichTextBuilder::new();
            builder.push(&msg)
            .add_attr(Attribute::font_family(FontFamily::MONOSPACE))
            .add_attr(Attribute::size(20.0));
            let msg = builder.build();

            bookState.setMsg(msg);
            

            let ui = Flex::<BookState>::column().with_child(RawLabel::new().lens(BookState::msg)).boxed();

            ctx.new_window(WindowDesc::new(ui)
            .title("Numero della Pagina Cartacea")
            //.set_position(center)
            .window_size((500.0 as f64, 100.0 as f64+10.0) ));

        })
        .padding(2.0).disabled_if(|bookState, _|

            bookState.getImagesPathSize() == 0 ||
            bookState.getCurrentView() == ViewState::EditMode ||
            bookState.getCurrentView()  == ViewState::HelpMode ||
            bookState.getCurrentView()  == ViewState::Idle)
}
...
#[allow(non_snake_case)]
fn sliderZoom() -> ControllerHost<LensWrap<BookState, f64, fontSize, SizedBox<f64>>, ZoomEvent> {

    let slider = Slider::new()
        .with_range(MAX_ZOOM, MIN_ZOOM)
        .track_color(KeyOrValue::Concrete(Color::RED))
        .knob_style(KnobStyle::Circle)
        .axis(Axis::Horizontal)
        .with_step(0.005)
        .fix_width(200.0)
        .lens(BookState::fontSize).controller(ZoomEvent);
    slider

}
#[allow(non_snake_case)]
fn reduceByZoom() -> Box<dyn druid::Widget<BookState>>{

    let label = Label::new("➖").on_click(|_, bookState: &mut BookState, _| {
        let currFontSize = bookState.getFontSize() - 0.05;
        if currFontSize <= MIN_ZOOM {
            bookState.setFontSize(MIN_ZOOM);
        }else {
            bookState.setFontSize(currFontSize);
        }
    }).controller(ZoomEvent).background(BLACK).rounded(ROUNDED_VALUE).boxed();
    label
}
#[allow(non_snake_case)]
fn magnifyByZoom() -> Box<dyn druid::Widget<BookState>> {

    let label = Label::new("➕").on_click(|_, bookState: &mut BookState, _| {
        let currFontSize = bookState.getFontSize() + 0.05;
        if currFontSize >= MAX_ZOOM {
            bookState.setFontSize(MAX_ZOOM);
        }else {
            bookState.setFontSize(currFontSize);
        }
    }).controller(ZoomEvent).background(BLACK).rounded(ROUNDED_VALUE).boxed();
...
pub fn toolbar () -> Flex<BookState> {
    ...
    .with_child(Flex::row()
        ...
        .with_child(refreshSearchButton())
        .with_spacer(BIG_SPACER)
        .with_child(ocrSearchButton())
        .with_default_spacer()
        .with_child(ocrReverseButton())
        .with_spacer(BIG_SPACER)
        .with_child(reduceByZoom())
        .with_default_spacer()
        .with_child(sliderZoom())
        .with_default_spacer()
        .with_child(magnifyByZoom())
        .align_left()
        .border(BLACK, 2.0)
        .rounded(ROUNDED_VALUE)
        .background(PURPLE), 
